...
def evaluate(model_name, query_list):
    ...
    if model_name == "simple":
        ...
        start = timeit.default_timer()
        res_dict = pbm.BIM_rank(query_list)
        stop = timeit.default_timer()
    elif model_name == "likelyhood":
        start = timeit.default_timer()
        res_dict = pbm.log_prob(query_list)
        stop = timeit.default_timer()
    else:
        print "no model named %s", model_name

    #sort res_dict by value
    if model_name not in ['unigram', 'likelyhood']:
...
unigram_dict, unigram_time = evaluate("unigram", query_term)
print "unigram:" + str(unigram_time)
